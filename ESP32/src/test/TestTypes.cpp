...
void TestTypes::runAllTests(TestRunner &runner) {
  ...
  // Point2D tests
  runner.runTest("Point2D: Default constructor",
                 testPoint2D_DefaultConstructor);
  runner.runTest("Point2D: Parameterized constructor",
                 testPoint2D_ParameterizedConstructor);
  runner.runTest("Point2D: Equality operator", testPoint2D_Equality);

  // JointAngles tests
  runner.runTest("JointAngles: Default constructor",
                 testJointAngles_DefaultConstructor);
  runner.runTest("JointAngles: Parameterized constructor",
                 testJointAngles_ParameterizedConstructor);

  // RobotState tests
  runner.runTest("RobotState: Default constructor",
                 testRobotState_DefaultConstructor);
  runner.runTest("RobotState: Initialization", testRobotState_Initialization);

  // Command tests
  runner.runTest("Command: Default constructor",
                 testCommand_DefaultConstructor);
...
bool TestTypes::testPoint2D_Equality() {
  ...
  return runner.assertTrue(p1 == p2) && runner.assertFalse(p1 == p3);
}
...
bool TestTypes::testRobotState_Initialization() {
  ...
  return runner.assertNear(150.0f, state.currentPosition.x, 0.01f) &&
         ...
         runner.assertTrue(state.isMoving) && runner.assertTrue(state.isHomed);
}
