            ...
            for e in msp:
                if e.dxftype() == 'LINE':
                    ...
                    if abs(x_start - x_end) > 0.001 or abs(y_start - y_end) > 0.001:
                        await self.send_command({"type": "TOOL_CONTROL", "state": False})
                        await self.send_command({"type": "MOVE_TO", "x": x_start, "y": y_start})
                        await self.send_command({"type": "TOOL_CONTROL", "state": True})
...
robot = RobotClient(ip="192.168.1.50", buffer_size=10)
...
async def on_button_click():
    ...
    await robot.upload_dxf_file("drawing.dxf")
