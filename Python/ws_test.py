...
class RobotWSClient:
    ...
    def __init__(self):
        ...
        # Flow control
        self.cmd_free = 30       # Slots libres (initialisé au max)
        self.mot_free = 1000     # Slots motion queue
        self.flow_event = asyncio.Event()
        self.flow_event.set()    # Initialement libre
        ...
        # File d'envoi pour le streaming DXF
    ...
    def _handle_message(self, raw: str):
        ...
        if msg_type == "ACK":
            ...
            if self.streaming:
                ...
                print(f"\r  {moving} {progress} {pos} {angles} cmdFree={self.cmd_free} motFree={self.mot_free}    ", end="", flush=True)

        elif msg_type == "ERROR":
            print(f"\n  ❌ ESP32 ERROR: {data.get('msg', 'unknown')}")

        else:
            print(f"\n  📩 {msg_type}: {json.dumps(data)}")

    def _update_flow_control(self):
        """Met à jour l'événement de flow control basé sur cmdFree."""
    ...
    async def stream_commands(self, commands: list):
        """
        Envoyer une liste de commandes avec flow control.
        Attend que cmdFree > seuil avant d'envoyer les prochaines commandes.
        """
        self.streaming = True
        ...
        print(f"     Flow control: seuil={FLOW_CONTROL_THRESHOLD}, batch={FLOW_CONTROL_BATCH}")
        ...
        try:
            ...
            while i < len(commands):
                ...
                # Envoyer un batch
                batch_size = min(FLOW_CONTROL_BATCH, len(commands) - i, self.cmd_free)
                batch_size = max(batch_size, 1)  # Au moins 1

                for j in range(int(batch_size)):
                    cmd = commands[i]
                    ok = await self.send_json(cmd)
                    if not ok:
                    ...
                    i += 1
                    self.stream_sent = i
                    self.cmd_free = max(0, int(self.cmd_free) - 1)
    ...
    def print_info(self):
        ...
        print(f"\n  Flow Control:")
        print(f"    cmdFree:   {self.cmd_free}")
        print(f"    motFree:   {self.mot_free}")
        print(f"    Seuil:     {FLOW_CONTROL_THRESHOLD}")
        print(f"    Batch:     {FLOW_CONTROL_BATCH}")
